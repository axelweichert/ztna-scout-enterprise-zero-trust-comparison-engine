      ...
      <div className={cn(
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="font-bold text-lg">Top Choice:</span>
            <Badge className="bg-primary">{sortedResults[0].vendorName}</Badge>
          </div>
          <Button size="sm" className="btn-gradient" onClick={() => navigate(`/vergleich/${id}/print`)}>
            <Printer className="mr-2 h-4 w-4" /> Export Report
          </Button>
        </div>
      </div>
      <div className="py-12 md:py-20 space-y-20">
        <header className="flex flex-col md:flex-row justify-between items-end gap-8 border-b border-primary/10 pb-12">
        ...
        <section className="space-y-8">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary" className="px-4 py-1">Enterprise Benchmark</Badge>
          </div>
          <Card className="p-10 shadow-2xl border-primary/5 bg-slate-50/30">
            <div className="h-[550px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ left: 100, right: 60 }}>
                  <CartesianGrid strokeDasharray="3 3" horizontal={false} opacity={0.1} />
                  <XAxis type="number" hide />
                  <YAxis dataKey="name" type="category" width={140} tick={{ fontSize: 13, fontWeight: 700, fill: 'hsl(var(--foreground))' }} />
                  <Tooltip 
                  ...
                  />
                  <Bar dataKey="tco" radius={[0, 8, 8, 0]} barSize={36}>
        ...
        {/* Feature Comparison Matrix for Top 3 */}
        <section className="space-y-8">
            ...
            <table className="w-full border-collapse">
                ...
                <tr className="bg-slate-50 border-b">
                  ...
                  {top3.map(v => (
                    <th key={v.vendorId} className="p-6 text-center">
                      ...
                      <Badge variant="outline" className="mt-2">Score: {v.scores.totalScore}</Badge>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    {top3.map(v => (
                      <td key={v.vendorId} className="p-6 text-center">
                        {(v.features as any)[key] ? (
                          <div className="bg-green-100 text-green-700 p-1.5 rounded-full inline-flex"><Check className="h-5 w-5" /></div>
                        ) : (
                          <div className="bg-red-50 text-red-300 p-1.5 rounded-full inline-flex"><X className="h-5 w-5" /></div>
                        )}
                      </td>
                    ))}
                  </tr>
                ...
                <tr className="bg-primary/5">
                  <td className="p-6 font-bold">BSI Certified</td>
                  {top3.map(v => (
                    <td key={v.vendorId} className="p-6 text-center">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {sortedResults.map((result) => (
            <Card
              key={result.vendorId}
              className={cn(
                "group relative overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 border-primary/5",
                result.vendorId === 'cloudflare' ? 'ring-2 ring-primary shadow-xl bg-primary/5' : 'bg-card shadow-lg'
              )}
            ...
            >
              <CardHeader className="pb-6">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <CardTitle className="text-2xl font-display">{result.vendorName}</CardTitle>
                    <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Scout Class Infrastructure</p>
                  </div>
                  <div className="flex flex-col items-end">
                    <Badge className={cn("px-3 py-1 text-sm font-bold", result.scores.totalScore > 80 ? "bg-green-600" : "bg-primary")}>
              ...
              <CardContent className="space-y-8">
                <div className="flex flex-col">
                  <span className="text-4xl font-bold tracking-tight">€{result.tcoYear1.toLocaleString()}</span>
                ...
                <div className="pt-6 border-t border-primary/10 space-y-6">
                  {result.features.isBSIQualified && (
                    <div className="flex items-center text-green-600 font-bold text-sm bg-green-50 p-2 rounded-lg">
                      <ShieldCheck className="w-5 h-5 mr-2" /> {t('results.bsi_qualified')}
                    </div>
                  )}
                  <div className="flex flex-wrap gap-2">
                    {result.features.hasZTNA && <Badge variant="secondary" className="bg-slate-200/50 text-slate-800 border-none">ZTNA</Badge>}
                    {result.features.hasSWG && <Badge variant="secondary" className="bg-slate-200/50 text-slate-800 border-none">SWG</Badge>}
                    {result.features.hasCASB && <Badge variant="secondary" className="bg-slate-200/50 text-slate-800 border-none">CASB</Badge>}
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="bg-slate-900 text-slate-100 rounded-3xl p-10 flex gap-8 items-center border shadow-2xl">
          <Info className="h-10 w-10 text-primary shrink-0" />
          <div className="space-y-2">
            <h4 className="text-xl font-bold">Important Data Transparency Notice</h4>
      ...
      <Dialog open={!!selectedVendor} onOpenChange={() => setSelectedVendor(null)}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader className="pb-4">
            <DialogTitle className="text-3xl font-display">{selectedVendor?.vendorName} Deep Dive</DialogTitle>
            <DialogDescription className="text-lg">Detailed performance breakdown and feature compliance.</DialogDescription>
          </DialogHeader>
          <div className="space-y-10 py-6">
            <div className="grid grid-cols-3 gap-6">
              {[
                { label: 'Price Score', val: selectedVendor?.scores.priceScore },
                { label: 'Feature Score', val: selectedVendor?.scores.featureScore },
                ...
                <div key={stat.label} className="p-4 bg-slate-50 border rounded-xl text-center">
                  ...
                  <p className="text-2xl font-bold">{stat.val}/100</p>
                </div>
              ))}
            </div>
            <div className="space-y-4">
              <h4 className="text-lg font-bold flex items-center gap-2"><Check className="text-primary w-5 h-5" /> Capability Checklist</h4>
              <div className="grid grid-cols-2 gap-3 text-sm">
                {Object.entries(selectedVendor?.features || {}).map(([key, val]) => (
                  key.startsWith('has') && <div key={key} className="flex justify-between p-3 bg-muted/30 rounded-lg border">
                    <span className="font-semibold text-foreground/70">{key.replace('has', '').toUpperCase()}</span>
                    <span className={val ? "text-green-600 font-bold" : "text-red-400 font-bold"}>{val ? 'ENABLED' : 'N/A'}</span>
                  </div>
                ))}
              </div>
            </div>
            <Button className="w-full btn-gradient py-6 text-lg" onClick={() => setSelectedVendor(null)}>Close Analysis</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
